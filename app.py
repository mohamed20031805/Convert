...
from formats_client.morgan_client import (
    charger_client,
    agréger_client,
    joindre,
    extraire_code_client
)
from formats_client.triton_client import (
    charger_client  as charger_client_triton,
    agréger_client  as agréger_client_triton,
    joindre         as joindre_triton
)
        ...
        with st.spinner("Extraction en cours..."):
            ...
            try:
                ...
                if not fmt:
                    ...
                    st.session_state.fichier_excel = exporter_excel(
                        output_df, entete, tmp_path
                    )
...
if st.session_state.extraction_done:
    ...
    st.success(
        f"✅ Extraction terminée ! **{len(st.session_state.lignes)} lignes** extraites."
    )
    ...
    # ── Fonction affichage comparaison ──
    ...
    if nom_format == "morgan":
        ...
        if client_file is not None:
            ...
            try:
                ...
                st.info(
                    f"🔑 Code client : **{code_client}** | Filtre PDF : **{code_pdf}**"
                )
    ...
    # ── BAML ──
        ...
        if client_file is not None:
            ...
            tmp_pdf_path = None
            try:
                # ── Détecter compte depuis nom fond Excel ──
                ...
                if account:
                    ...
                    st.info(
                        f"🔑 Compte détecté : **{account_number}** "
                        f"— {account['client_name']}"
                    )
                else:
                    account_number = None
                    st.warning(
                        "⚠️ Compte non trouvé dans accounts.json "
                        "— toutes les pages extraites"
                    )
                ...
                lignes_filtrees  = baml.extraire_positions(
                    tmp_pdf_path,
